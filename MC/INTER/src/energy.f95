...
contains
 ...
 subroutine ENERGYSYS(Usys,NPart,SLengthx,Slengthy,NVList,VList,TYP,X,Y)
	implicit none
	...
	real(dp), intent(in) :: SLengthx		! Input, Box Length x	
	real(dp), intent(in) :: SLengthy		! Input, Box Length y
 ...
 subroutine ENERGYPART(Upart,Part,SLengthx,Slengthy,VListP,TYP,X,Y,XP,YP)
	implicit none
	...
	do jNeigh=1,size(VListP)
		jPart = VListP(jNeigh)
		if (jPart .ne. 0) then
			XD = XP-X(jPart) - SLengthx*NInt((XP-X(jPart))/SLengthx)  ! Distance adjusting for PBC
			YD = YP-Y(jPart) - SLengthy*NInt((YP-Y(jPart))/SLengthy)  ! Distance adjusting for PBC					
			R = sqrt(XD**2 + YD**2) ! Compute distance
