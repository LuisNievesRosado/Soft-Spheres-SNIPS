module verletlists
 ...
 subroutine MAKELIST(VList,NVList,NVMax,VCut,HOC,LIS,CELLNEIGH,NCell1,NPart,SLength,X,Y)
	implicit none
	...
	integer, intent(in) :: NCell1 		   ! Input, Number of Cells per direction
	...
	part_loop: do iPart = 1,NPart
		...
		Cell = CEILING((X(iPart) + SLength/2)*NCell1/SLength) + NCell1*(CEILING((Y(iPart) + SLength/2)*NCell1/SLength) - 1)
 ...
 subroutine UPTLISTDISP(Part,VList,NVList,VCut,SLength,NCell1,HOC,LIS,CELLNEIGH,X,Y)
	implicit none
	...
	real(dp), intent(in) :: SLength				! Input, Length of Box Size
	...
	Cell = CEILING((X(Part) + SLength/2)*NCell1/SLength) + NCell1*(CEILING((Y(Part) + SLength/2)*NCell1/SLength) - 1)
	NVList(Part) = 0
	VList(Part,:) = 0
